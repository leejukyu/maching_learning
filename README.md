...
#### (2) 회귀
- 평균절대오차(MAE) : 실제값과 예측값의 차이를 절댓값으로 변환해 평균, **이상치**로 보이는 구역이 많은 회귀문제
- 평균제곱오차(MSE) : 실제값과 예측값의 차이를 제곱해 평균, 회귀문제의 전형적인 성능지표
- RMSE : MSE는 실제 오류 평균보다 커지는 특성이 있으므로 루트를 씌움
- R^2 : 분산 기반의 예측 성능 평가, 1에 가까울수록 정확도가 높음
### 3) 가정검사
## 2. 데이터를 구한다
## 3. 탐색 및 시각화
...
### [3) 앙상블(ensemble learning)](ensemble.ipynb)
- 예측기로부터 예측을 수집한 모델
...
- 부스팅(boosting) : 약한 학습기를 여러 개 연결하여 강한 학습기를 만드는 방법
  ...
  - GBM(gradient) : 가중치 업데이트를 경사하강법으로, 회귀도 가능, 랜덤 포레스트보다 성능이 뛰어난 경우가 많지만 오래걸림
    ...
    - LightGBM : XGBoost보다 빠르고 메모리 사용량 적음, 데이터가 작으면 과적합 위험, 원핫인코딩 없이도 최적 분할 가능
### 4) [회귀(regression)](Regression.ipynb)
- 경사하강법 : 미분을 적용하여 기울기가 감소하지 않는 지점인 비용 함수가 최소인 점을 반환 -> 회귀 계수
- 선형회귀(linear regression) : 예측값과 실제값의 RSS를 최소화해 OLS 추정 방식, 규제 적용 안함
  - 다항회귀 : 다항식의 차수가 높아질수록 과적합 문제, 너무 낮으면 과소적합
  - 편향-분산 트레이드오프 : 편향이 높으면 전체 오류가 높음(과소적합), 편향을 낮추면 분산이 높아지고 오류도 낮아짐(과적합)
- 규제 : 과대적합 감소, 훈련이 끝나면 규제가 없는 성능 지표로 평가, 릿지가 기본이지만 특성이 몇 개 뿐이면 라쏘나 엘라스틱넷
  - 릿지 회귀(ridge, L2)
    - 규제항(노름의 제곱을 2로 나눈 것)을 비용 함수에 추가하여 모델의 가중치가 가능한 작게 유지
    - 하이퍼 파라미터 알파는 모델을 얼마나 많이 규제할지 조절, 클수록 회귀계수가 작아짐, 0되지는않음
  - 라쏘 회귀(lasso, L1)
    - 규제항(가중치 벡터의 노름)을 비용 함수에 추가하여 덜 중요한 특성의 가중치를 제거
  - 엘라스틱넷(elastic net)
    - 릿지와 라쏘 절충 모델, 라쏘 회귀의 급격한 변동을 완화하기 위해 라쏘 회귀를 추가
  - 로지스틱회귀(logistic regression)
    - 회귀 방정식을 이용한 **분류** 알고리즘, 이진 분류에 뛰어남
    - s자의 최적선을 찾고 함수의 반환 값을 확률로 간주해 확률에 따라 분류를 결정
  - 회귀트리
    - **비선형 회귀**, 모든 트리기반 알고리즘은 분류뿐만 아니라 회귀도 가능
...
- 부스터 파라미터 : 트리 최저고하, 부스팅, regularization 등과 관련 파라미터
  - learning_rate : 학습률, 0~1사이, 디폴트-0.1, 0.01~0.2 사이의 값 선호
  ...
  - reg_lambda : L2 Regularization 적용 값, 피처가 많을수록 적용 검토, 값이 클수록 과적합 감소
  - scale_pos_weight : 디폴트-1, 비대칭 클래스 데이터 셋의 균형을 유지하기 위해 사용
- 학습 태스크 파라미터 : 학습 수행 시의 객체 함수, 평가를 위한 지표 등
- 과적합 문제가 심할 때 : eta, max_depth - 낮춤, min_child_weight, gamma - 높임
### 4) 회귀
- fit_intercept : 절편값을 계산 할지 말지, 디폴트-True
- normalize : fit_intercept가 False면 사용안함, 회귀 수행 전 데이터 셋 정규화, 디폴트-False
- coef_ : fit 후 회귀 계수가 배열 형태로 저장하는 속성,(Target값 개수, 피처 개수)
- intercept_ : intercept값
- (로지스틱) penalty : 규제
- (로지스틱) C : 규제 강도를 조절하는 alpha값의 역수, 작을 수록 규제 강도가 큼
### 5) 학습 파라미터
- binary:logistic : 이진분류
- multi:softmax : 다중 분류, num_class파라미터 지정
- multi:softprob : 개별 레이블 클래스의 예측 확률 반환
- eval_metric : 검증에 사용되는 함수, 회귀인 경우 rmse, 분류일 경우 error
  - rmse
  - mae
  - logloss
  - error
